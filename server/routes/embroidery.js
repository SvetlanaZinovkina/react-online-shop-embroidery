...
export default (app) => {
		...
		app.get('/api/v1/catalog/category/:categoryId', async (req, reply) => {
				try {
						const { page = 1, limit = 20 } = req.query;
						const offset = (page - 1) * limit;
		...
		app.post('/api/v1/catalog', async (req, reply) => {
				...
				try {
						const [productId] = await knex('products').insert({
								name: title,
								description,
								price: 0, // Assuming price is not provided here, adjust as needed
								file_path: file.path,
